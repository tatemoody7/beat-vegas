...
def _oof_preds(df: pd.DataFrame, cols: List[str], first_test_season: int = 2018,
               min_train: int = 500) -> Optional[pd.DataFrame]:
    keep = (["id", "season", "week", "under", "proxy_line", "first_half_total",
             "full_game_total"] + _SEG_COLS + cols)
...
def _summ(g: pd.DataFrame) -> Dict:
    return {"n": int(len(g)), "under_pct": round(100 * g["under"].mean(), 1),
            "roi": round(_roi(int(g["under"].sum()), len(g)), 4)}


def inspect_combo(df: pd.DataFrame, cols: List[str], top_frac: float = 0.20,
                  first_test_season: int = 2018) -> Dict:
    ...
    by_season = [{"season": int(s), **_summ(g)}
                 for s, g in sel.groupby("season")]
    era = {"pre2023 (<=2022)": _summ(sel[sel["season"] <= 2022]),
    ...
    for c in _SEG_COLS:
        if c in sel.columns and sel[c].notna().any():
            segs[c] = {str(int(v)) if pd.notna(v) else "na": _summ(g)
                       for v, g in sel.groupby(c)}
    ...
    for d in (-2.0, -1.0, -0.5, 0.0, 0.5, 1.0, 2.0):
        ...
        stress.append({"delta": d, "n": int(len(s)),
    ...
    for c in cols:
        if c in pg.columns:
            direction[c] = {"selected_mean": round(float(sel[c].mean()), 3),
                            "pool_mean": round(float(pg[c].mean()), 3)}

    return {"factors": cols, "top_frac": top_frac,
            "overall": _summ(sel), "by_season": by_season,
            "by_era": era, "recent": recent, "by_segment": segs,
            "proxy_stress": stress, "direction": direction}
