"""Best-effort college-football NEWS from ESPN's undocumented API.

DISPLAY CONTEXT ONLY — never a model feature. The API is unofficial (can change
without notice), so every call fails silent (returns empty) rather than raising.
ESPN publishes NO college-football injuries (0 for every FBS team, verified
2026-09-02) — injuries and the QB-out flag come from beatvegas.sources.rotowire.
"""
