...
def main() -> None:
    ...
    with session_scope() as sess:
        if args.cmd == "set":
            ...
            sess.add(BvAdjustment(game_id=args.game, delta_pts=args.delta,
                                  reason=args.reason, created_at=datetime.utcnow()))
            ...
            rows = (sess.query(BvAdjustment, Game)
                    .outerjoin(Game, Game.id == BvAdjustment.game_id)
                    .order_by(BvAdjustment.created_at.desc()).all())
            if not rows:
                print("no adjustments set")
            for adj, g in rows:
                matchup = f"{g.away_team} @ {g.home_team}" if g else "?"
                print(f"  game {adj.game_id} ({matchup}): {adj.delta_pts:+g} "
                      f"— {adj.reason or ''}")
