...
def main() -> None:
    ...
    if len(df) < 200:
        print(
            f"Only {len(df)} games carry a spread — run scripts/backfill.py to "
    ...
    for ts in seasons:
        ...
        fit_pred.extend(
            _predict_line(test["full_game_total"].to_numpy(), test["spread"].to_numpy(), coeffs)
        )
        flat_pred.extend(np.round(test["full_game_total"].to_numpy() * DEFAULT_SHARE * 2) / 2)
    ...
    final = fit_share(
        df["full_game_total"].to_numpy(), df["spread"].to_numpy(), df["first_half_total"].to_numpy()
    )
    print(
        f"walk-forward MAE (1H line vs realized): "
        f"spread-adjusted={fit_mae:.3f}  flat-0.52={flat_mae:.3f}  "
        f"n={len(actuals)}"
    )
    print(
        f"fitted share = {final['a']:.4f} + {final['b']:.5f}*|spread|  (clamped to {SHARE_CLAMP})"
    )
    for sp in (0, 7, 14, 21):
