...
def main() -> None:
    ...
    out.sort(key=lambda x: (x["week"] is None, x["week"] or 0,
                            f"{x['away']} @ {x['home']}"))

    scope = f"wk{args.week}" if args.week is not None else "all upcoming"
    print(f"source={source} season={args.season} {scope} — {len(out)} games "
          f"(derived 1H from posted line; not a pick, not graded)\n")
    print(f"{'wk':>3}  {'matchup':42}  {'FG tot':>6}  {'spread':>6}  "
          f"{'1H':>5}  {'share':>5}")
    print("-" * 80)
    for r in out:
        ...
        print(f"{wk:>3}  {matchup:42.42}  {r['total']:>6.1f}  {sp:>6}  "
