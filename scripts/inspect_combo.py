...
def main() -> None:
    ...
    rep = inspect_combo(df, cols, top_frac=args.top_frac, first_test_season=args.first_test_season)
    if args.json:
    ...
    print(f"OVERALL: {_line(rep['overall'], ['n', 'under_pct', 'roi'])}  (breakeven 52.4%)")
    print("\nBy season:")
    ...
    for seg, m in rep["by_segment"].items():
        parts = "  |  ".join(
            f"{kk}: {vv['under_pct']}% roi={vv['roi']} n={vv['n']}" for kk, vv in m.items()
        )
        print(f"  {seg}: {parts}")
