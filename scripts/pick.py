...
def _season_games(s, season: int) -> List[dict]:
    rows = s.query(Game.id, Game.week, Game.home_team, Game.away_team).filter(
        Game.season == season).all()
    return [{"id": r[0], "week": r[1], "home_team": r[2], "away_team": r[3]}
            for r in rows]
...
def cmd_add(args) -> None:
    ...
    with session_scope() as s:
        ...
        if gid is None:
            print(f"No game found for '{args.away} @ {args.home}' in {season}. "
        ...
        pick = ManualPick(
            ...
            side="under", line=args.line, price=args.price, stake=args.stake,
            book=args.book, placed_at=datetime.utcnow(), note=args.note,
            graded=False,
        )
        s.add(pick)
        s.flush()
        print(f"logged pick #{pick.id}: UNDER {args.line} ({args.price}) "
...
def cmd_list(args) -> None:
    with session_scope() as s:
        ...
        if not rows:
            ...
            return
        for p in rows:
            status = (f"{p.result} ({p.units:+.2f}u, CLV {p.clv:+.1f})"
                      if p.graded else "pending")
            print(f"#{p.id} [{p.season} wk{p.week}] UNDER {p.line} {p.price} "
...
def cmd_grade(args) -> None:
    ...
    graded = 0
    with session_scope() as s:
        picks = s.query(ManualPick).filter(
            ManualPick.season == season, ManualPick.graded == False,  # noqa: E712
            ManualPick.game_id.isnot(None)).all()
        for p in picks:
            ...
            snaps = s.query(OddsSnapshot).filter(
                OddsSnapshot.game_id == p.game_id,
...
def cmd_summary(args) -> None:
    with session_scope() as s:
        ...
        print(f"YOUR RECORD: {wins}-{decided-wins}"
