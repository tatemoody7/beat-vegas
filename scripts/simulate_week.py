...
def main() -> None:
    ...
    ap.add_argument("--week", type=int, default=8,
                    help="default 8 (full model board); use 1 for cold-start")
    ap.add_argument("--limit", type=int, default=12,
                    help="max games to include (cheapest-total first); 0 = all")
    ...
    if args.grade:
        res = run_pipeline("sim", args.season, args.week, mode="replay",
                           action="grade")
        print(f"[reveal] market={res['market']} model={res['model']} "
              f"your_picks={res['your_picks']} — check the Ledger + My Picks.")
        return

    print(f"[sim] local Postgres up: {uri}")
    res = run_pipeline("sim", args.season, args.week, mode="replay",
                       notify="print", limit=(args.limit or None))

    print(f"[sim] cloned input rows: "
          + ", ".join(f"{k}={v}" for k, v in res["cloned"].items()))
    print(f"[sim] PRE-GAME slate: {res['games']} games | {res['model_preds']} "
