...
on a single requests.ReadTimeout. 3 attempts, backoff 2s then 4s, on timeout /
...
def test_gives_up_after_three_attempts(monkeypatch):
    c, sleeps = _client(
        [requests.ReadTimeout("1"), requests.ReadTimeout("2"), requests.ReadTimeout("3")],
        monkeypatch,
    )
    with pytest.raises(requests.ReadTimeout):
        c.games(2026)
    assert c._session.calls == 3
    assert sleeps == [2, 4]  # no sleep after the final failure


def test_persistent_5xx_raises_http_error(monkeypatch):
    c, _ = _client([_Resp(500), _Resp(502), _Resp(503)], monkeypatch)
